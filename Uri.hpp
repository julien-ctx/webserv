...
Uri() {}
Uri(std::string path) : _path(path) { SetPathToLowercase(); }
~Uri() {}
...
{
    std::transform(method_string.begin(), method_string.end(), std::back_inserter(method_string_lowercase),
                [](char c) { return tolower(c); }); // --> voir utils.hpp pour std::transform
}
